...
const handler = async (m, {command, usedPrefix, text}) => {
  ...
  if (!text) throw `${tradutor.texto2[0]} *${usedPrefix}list${which}* ةة ${tradutor.texto2[1]}`;
  const msgs = global.db.data.msgs;
  if (text in msgs) throw `*[❗𝐈𝐍𝐅𝐎❗] '${text}'لا ${tradutor.texto3}`;
  msgs[text] = M.toObject(await m.getQuotedObj());
  m.reply(`${tradutor.texto4[0]} '${text}'${tradutor.texto4[1]} ${usedPrefix}استدعاء${which} ${text}*`);
};

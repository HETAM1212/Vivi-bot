...
return conn.reply(m.chat, tradutor.texto2, m);
  ...
  const fakegif = {key: {participant: `0@s.whatsapp.net`, ...('6289643739077-1613049930@g.us' ? {remoteJid: '6289643739077-1613049930@g.us'} : {})}, message: {'videoMessage': {'title': 'Vivi Bot MD', 'h': `Hmm`, 'seconds': '99999', 'gifPlayback': 'true', 'caption': 'vivi bot', 'jpegThumbnail': false}}};
  const teks = `${tradutor.texto4[1]}${name}\n*${tradutor.texto4[2]} wa.me/${who.split`@`[0]}\n*${tradutor.texto4[3]} ${text}`;
  for (const id of groups) {
    await conn.sendMessage(id, {text: teks}, {quoted: fakegif});
    global.db.data.users[m.sender].msgwait = new Date * 1;
  }
};
handler.command = /^(نشر2)$/i;
handler.owner = true;
handler.group = true;
